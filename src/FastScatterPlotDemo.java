...
public class FastScatterPlotDemo extends ApplicationFrame implements Constants {
    ...
    public FastScatterPlotDemo(final String title) {
        ...
        domainAxis.setRange(-5.00,5.00);
        final NumberAxis rangeAxis = new NumberAxis("Y");
        rangeAxis.setRange(-5.00,5.00);
    ...
    public void populateData() {
        ...
        PSOProcess p = new gBestPSO(1);
        p.initialise();
        Double[] results = p.execute();
        for(int i=0; i<numIterations; i++) {
            double d0 = p.swarm.get(i).getP()[0];
            double d1 = p.swarm.get(i).getP()[1];
            this.data[0][i] = (float)d0;
            this.data[1][i] = (float)d1;
